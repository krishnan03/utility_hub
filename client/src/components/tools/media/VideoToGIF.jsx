...
export default function VideoToGIF() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      fd.append('duration', Math.min(Number(duration), 10).toString());
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="configure" variants={stepVariants} initial="enter" animate="center" exit="exit" className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" value={startTime} onChange={(e) => setStartTime(e.target.value)} min="0" step="0.1" className="w-full px-3 py-2.5 rounded-xl text-sm text-surface-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-surface-300 mb-1.5">Duration (s, max 10)</label>
                <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} min="1" max="10" className="w-full px-3 py-2.5 rounded-xl text-sm text-surface-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }} />
              </div>
            </div>

            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1.5">Width (px)</label>
              <div className="flex gap-2">
                {['320', '480', '640'].map((w) => (
                  <button key={w} type="button" onClick={() => setWidth(w)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${width === w ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={width === w ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{w}px</button>
                ))}
              </div>
            </div>

            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1.5">Frame Rate</label>
              <div className="flex gap-2">
                {['5', '10', '15'].map((f) => (
                  <button key={f} type="button" onClick={() => setFps(f)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${fps === f ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={fps === f ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{f} fps</button>
                ))}
              </div>
            </div>
        ...
        {processing && (
          <motion.div key="processing" variants={stepVariants} initial="enter" animate="center" exit="exit" className="py-8">
            <ProgressBar progress={progress} label="Creating GIF..." indeterminate />
          </motion.div>
        )}
        ...
        {step === 'done' && result && (
          <motion.div key="done" variants={stepVariants} initial="enter" animate="center" exit="exit" className="space-y-4">
            ...
            <div className="text-center"><button type="button" onClick={reset} className="text-sm font-medium text-primary-500 hover:text-primary-400 transition-colors">Convert another video →</button></div>
          </motion.div>
        )}
      </AnimatePresence>
