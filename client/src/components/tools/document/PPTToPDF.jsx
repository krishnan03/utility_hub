      ...
      <AnimatePresence mode="wait">
        {step === 'upload' && (
          <motion.div key="upload" variants={stepVariants} initial="enter" animate="center" exit="exit">
            <FileUpload onFilesSelected={(f) => setFile(f[0])} accept=".ppt,.pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation" multiple={false} />
          </motion.div>
        )}
        {step === 'configure' && !processing && (
          <motion.div key="configure" variants={stepVariants} initial="enter" animate="center" exit="exit" className="space-y-5">
            ...
            <div className="p-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <p className="text-xs text-surface-400 mt-1">Each slide will become a page in the PDF</p>
            </div>
        ...
        {processing && (
          <motion.div key="processing" variants={stepVariants} initial="enter" animate="center" exit="exit" className="py-8">
            <ProgressBar progress={progress} label="Converting slides..." indeterminate />
          </motion.div>
        )}
        {step === 'done' && result && (
