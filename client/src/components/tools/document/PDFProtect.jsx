...
export default function PDFProtect() {
  ...
  const handleSubmit = async () => {
    if (!file) return;
    if (action === 'add' && !password) { setError('Please enter a password'); return; }
    setProcessing(true);
    setError(null);
    try {
      ...
      const res = await fetch('/api/pdf/protect', { method: 'POST', body: fd, credentials: 'include' });
  ...
  const reset = () => { setFile(null); setResult(null); setError(null); setPassword(''); };
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="configure" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[['add', '🔒 Add password'], ['remove', '🔓 Remove password']].map(([val, label]) => (
                  <button key={val} type="button" onClick={() => setAction(val)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${action === val ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={action === val ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{label}</button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-surface-300 mb-1">
                {action === 'add' ? 'New password' : 'Current password (to unlock)'}
              </label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password..." className="w-full px-4 py-3 pr-12 rounded-xl text-sm text-surface-100 placeholder-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500/40" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-surface-400 hover:text-surface-600 dark:hover:text-surface-300 transition-colors">
                  {showPassword ? (
            ...
            <motion.button type="button" onClick={handleSubmit} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} className="w-full min-h-[52px] rounded-2xl text-base font-bold bg-gradient-to-r from-primary-600 to-primary-500 text-white shadow-lg shadow-primary-500/20 hover:shadow-xl transition-all duration-200">
              {action === 'add' ? 'Protect PDF' : 'Unlock PDF'}
            </motion.button>
          </motion.div>
        )}
        ...
        {processing && (
          <motion.div key="processing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-8">
            <ProgressBar label="Processing PDF..." indeterminate />
          </motion.div>
        )}
        ...
        {step === 'done' && result && (
          <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4">
            <div className="flex flex-col items-center gap-4 p-6 rounded-3xl" style={{ background: 'linear-gradient(135deg, rgba(255,99,99,0.08), rgba(16,185,129,0.08))', border: '1px solid rgba(255,99,99,0.15)' }}>
              ...
              <p className="text-sm font-semibold text-surface-100">{action === 'add' ? 'PDF protected!' : 'PDF unlocked!'}</p>
