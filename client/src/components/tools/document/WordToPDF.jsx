  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Server returned an invalid response. Word to PDF conversion may not be supported on this server.');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-xs text-surface-400 flex items-center gap-1.5">
        ...
        Word to PDF conversion requires server processing
      </p>
      <AnimatePresence mode="wait">
        {step === 'upload' && (
          <motion.div key="upload" variants={stepVariants} initial="enter" animate="center" exit="exit">
            <FileUpload onFilesSelected={(f) => setFile(f[0])} accept=".docx,.doc,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword" multiple={false} />
          </motion.div>
        )}
        ...
        {step === 'configure' && !processing && (
          <motion.div key="configure" variants={stepVariants} initial="enter" animate="center" exit="exit" className="space-y-5">
            ...
            <div className="p-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
              <p className="text-sm font-semibold text-surface-300">Output: PDF</p>
              <p className="text-xs text-surface-400 mt-1">Your Word document will be converted to a PDF file</p>
            </div>
