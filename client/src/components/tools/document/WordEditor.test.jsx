...
describe('WordEditor', () => {
  ...
  it('clears localStorage on New Document', async () => {
    localStorage.setItem('utilityhub_word_editor', JSON.stringify({ title: 'Old', content: {} }));
    const user = userEvent.setup();
    renderEditor();
    ...
    expect(localStorage.getItem('utilityhub_word_editor')).toBeNull();
  });
  ...
  it('restores content from localStorage on mount', async () => {
    ...
    localStorage.setItem('utilityhub_word_editor', JSON.stringify(savedData));
  ...
  it('saves to localStorage on Ctrl+S', async () => {
    ...
    await waitFor(() => {
      const saved = localStorage.getItem('utilityhub_word_editor');
