...
export default function PDFConverter() {
  ...
  const [direction, setDirection] = useState('pdf-to-images');
  ...
  const accept = direction === 'images-to-pdf' ? 'image/*' : '.pdf,application/pdf';
      ...
      <AnimatePresence mode="wait">
        {step === 'upload' && (
          <motion.div key="upload" variants={stepVariants} initial="enter" animate="center" exit="exit">
            <div className="mb-4">
              <label className="block text-sm font-semibold text-surface-300 mb-2">Conversion direction</label>
              <div className="grid grid-cols-3 gap-2">
                {[['pdf-to-images', 'PDF → Images'], ['images-to-pdf', 'Images → PDF'], ['pdf-to-text', 'PDF → Text']].map(([val, label]) => (
                  <button key={val} type="button" onClick={() => setDirection(val)} className={`py-2.5 px-3 rounded-xl text-xs font-bold transition-all duration-200 ${direction === val ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={direction === val ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{label}</button>
                ))}
              </div>
            </div>
            <FileUpload onFilesSelected={(f) => setFile(f[0])} accept={accept} multiple={false} />
          </motion.div>
        )}
        ...
        {step === 'configure' && !processing && (
          <motion.div key="configure" variants={stepVariants} initial="enter" animate="center" exit="exit" className="space-y-5">
            ...
            {direction === 'pdf-to-images' && (
              <>
                <div>
                  <label className="block text-sm font-semibold text-surface-300 mb-2">Output format</label>
                  <div className="flex gap-2">
                    {['png', 'jpg'].map((f) => (
                      <button key={f} type="button" onClick={() => setOutputFormat(f)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold uppercase transition-all duration-200 ${outputFormat === f ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={outputFormat === f ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{f}</button>
                    ))}
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-surface-300 mb-2">DPI (resolution)</label>
                  <div className="flex gap-2">
                    {['72', '150', '300'].map((d) => (
                      <button key={d} type="button" onClick={() => setDpi(d)} className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ${dpi === d ? 'text-white' : 'text-surface-300 hover:bg-white/5'}`} style={dpi === d ? { background: 'linear-gradient(135deg, #FF6363, #FF9F43)' } : { background: 'rgba(255,255,255,0.06)' }}>{d}</button>
                    ))}
                  </div>
                </div>
              </>
        ...
        {processing && (
          <motion.div key="processing" variants={stepVariants} initial="enter" animate="center" exit="exit" className="py-8">
            <ProgressBar progress={progress} label="Converting..." indeterminate />
          </motion.div>
        )}
